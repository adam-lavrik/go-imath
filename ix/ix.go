...
func Absu(value int) uint {
	mask := value >> (BitSize - 1)
	return uint(value ^ mask) + uint(mask & 1)
}

func Copysign(target, source int) int {
	if target == 0 {
		return 0
	}
...
func GCD(value_0, value_1 int) uint {
	for value_1 != 0 {
...
func LCM(value_0, value_1 int) uint {
...
func MinSliceChecked(values []int) (int, bool) {
	if len(values) == 0 {
		return 0, true
	}
...
func MaxSliceChecked(values []int) (int, bool) {
	if len(values) == 0 {
		return 0, true
	}
...
func MinMax(value_0, value_1 int) (int, int) {
	if value_0 < value_1 {
...
func MinMaxs(value int, values ...int) (int, int) {
	min := value
	max := value
	for _, v := range values {
		if v < min {
...
func MinMaxSlice(values []int) (int, int) {
	return MinMaxs(values[0], values[1:]...)
}

func MinMaxSliceChecked(values []int) (int, int, bool) {
	if len(values) == 0 {
		return 0, 0, true
	}
